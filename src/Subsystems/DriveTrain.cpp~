...
bool DriveTrain::TurnTo(float degrees){		  		   //Slant comments are cool
	...
	if(error > 1)						 		   //makes it so motors don't break by inputing a float higher than the motors can take
		...
		error = -1;							    //
	mecanum->MecanumDrive_Cartesian(0,0,error);//only so the robot turns not move forward
	if(error < .5 && error > .5)   			  //if error is within a reasonable range say basically it's close enough
		finished = true;
	return finished;
}
